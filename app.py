...
def generate_recommendations(user_data, tokenizer, model, max_length=500):
    """
...
- Trade/Business Income: {user_data['trade_business_income']}
- Dividends: {user_data['dividends']}
- Interest: {user_data['interest']}
- Rent from Property: {user_data['rent']}
...
- CPF Relief: ${user_data['cpf_relief']} (max: $8,000)
    ...
    try:
        ...
        if content.startswith('[') and content.endswith(']'):
            ...
            recommendations = [
                {
                    "Relief Name": "CPF Top-up",
                    "Suggested Amount": min(8000 - user_data['cpf_relief'], 8000),
                    "Tax Deduction": min(8000 - user_data['cpf_relief'], 8000),
                    "Current Tax": current_tax,
                    "New Tax": calculate_tax(user_data['chargeable_income'] - min(8000 - user_data['cpf_relief'], 8000)),
                    "Tax Savings": current_tax - calculate_tax(user_data['chargeable_income'] - min(8000 - user_data['cpf_relief'], 8000)),
                    "Rationale": f"Maximize your CPF relief by contributing ${min(8000 - user_data['cpf_relief'], 8000):,.2f}. This reduces your tax while securing your retirement."
                },
                {
...
def children_info():
    ...
    if form.validate_on_submit():
        ...
        for child_form in form.children:
            child_data = {
                'name': child_form.child_name.data,
...
def results():
    ...
    print(f"Trade/business income: {session.get('trade_business_income', 0)}")
